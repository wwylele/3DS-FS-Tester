...
ifeq ($(strip $(MAKEROM)),)
$(error "Please set MAKEROM in your environment. export MAKEROM=<path to>makerom")
endif
...
$(OUTPUT).cxi   :   $(OUTPUT).elf $(TOPDIR)/icon.icn $(TOPDIR)/rsf.rsf $(TOPDIR)/banner.bnr
	@$(MAKEROM) -o $(OUTPUT).cxi -rsf $(TOPDIR)/rsf.rsf -target t -elf $(OUTPUT).elf -icon $(TOPDIR)/icon.icn -banner $(TOPDIR)/banner.bnr -desc app:4
